...
function calls($reportdata, $id, $k)
{
  ...
  // echo $k."---".$reportdata[$id][$k]["Start Time"]."---".$reportdata[$id][$k]["End Time"]."<br>";
        $logdate=date('Y-m-d',strtotime($reportdata[$id][$k]["Start Time"]));
        $prevlogdate = strtotime(date('Y-m-d', strtotime('-7 day')));
        if ($logdate > $prevlogdate) {
            $table = DB::table('crmcalls');
        } else {
            $table = DB::table('crmcalls_archive');
        }
    $calls = $table->where("user_id", $id)->where('created_at', ">=", $reportdata[$id][$k]["Start Time"])->where('updated_at', "<=", $reportdata[$id][$k]["End Time"])->where('sipid_id', '>', '0')->select('created_at', 'updated_at', 'number', 'crm_id', 'ts_Wait', 'ts_Close', 'ts_Force', 'id', 'type', 'ts_Call', 'crm_id', "userstatus", "state", "sipid_id")->orderBy('created_at', 'asc')->get();
    if (!empty($calls[0])) {
        // echo "--data<br>";
        $lastCall_id = 0;
        ...
        if (!empty($lastCall)) {
            ...
            if (array_key_exists("callid", $reportdata[$id][$lastCall_key])) {
                ...
                if ($lastCall_id > 0) {
                    $calls2  = $table->where("user_id", $id)->where('id', ">", $lastCall_id)->where('id', "<", $calls[0]->id)->whereNotIn('id', [$lastCall_id, $calls[0]->id])->where('sipid_id', '>', '0')->select('created_at', 'updated_at', 'number', 'crm_id', 'ts_Wait', 'ts_Close', 'ts_Force', 'id', 'type', 'ts_Call', 'crm_id', "userstatus", "state", "sipid_id")->orderBy('created_at', 'asc')->get();
                    $result1 = array();
                    if (!empty($calls2[0])) {
    ...
    if (!empty($calls[0])) {
        ...
        foreach ($calls as $cval) {
            ...
            if ($call_mode == "Inbound") {
                $searchkey = (string) array_search(date("Y-m-d H:i:s", round($cval->ts_Wait,2)/ 1000), array_column($reportdata[$id], 'Start Time'));
                if ($searchkey != "") {
                    if ($reportdata[$id][$searchkey]["Status"] == "Refresh Page") {
                        $reportdata[$id][$searchkey]['unset']="Y1";
                    }
                }
                $reportdata[$id][$k]["Start Time"]   = date("Y-m-d H:i:s", round($cval->ts_Wait,2)/ 1000);
                $reportdata[$id][$k]["Mode"]         = $call_mode;
                $reportdata[$id][$k]["Status"]       = "Wait";
                $reportdata[$id][$k]["End Time"]     = date("Y-m-d H:i:s", round($cval->ts_Call,2)/ 1000);
                ...
                $k                                   = $k + 1;
                $reportdata[$id][$k]["Start Time"]   = date("Y-m-d H:i:s", round($cval->ts_Call,2)/ 1000);
                $reportdata[$id][$k]["Mode"]         = $call_mode;
            ...
            if (!empty($cval->ts_Force)) {
                $reportdata[$id][$k]["End Time"]     = date("Y-m-d H:i:s", round($cval->ts_Force,2)/ 1000);
                $event_duration                      = date("H:i:s", (strtotime($reportdata[$id][$k]["End Time"]) - strtotime($reportdata[$id][$k]["Start Time"])));
                $reportdata[$id][$k]["Event Length"] = $event_duration;
            } else {
                $reportdata[$id][$k]["End Time"]     = date("Y-m-d H:i:s", round($cval->ts_Close,2)/ 1000);
                $event_duration                      = date("H:i:s", (strtotime($reportdata[$id][$k]["End Time"]) - strtotime($reportdata[$id][$k]["Start Time"])));
                $reportdata[$id][$k]["Event Length"] = $event_duration;
            }
            $c++;
            ...
            if (isset($sdata['states']) && !empty($sdata['states'])) {
                ...
                if (!empty($newarr)) {
                    ...
                    if ($j == 0) {
                        ...
                        $reportdata[$id][$k]["End Time"]     = date("Y-m-d H:i:s", round($firstts,2)/ 1000);
                        $reportdata[$id][$k]["Client Id"]    = "";
                        $reportdata[$id][$k]["Number"]       = "";
                        $reportdata[$id][$k]["User Name"]    = $reportdata[$id][0]["User Name"];
                        $reportdata[$id][$k]["User Id"]      = $reportdata[$id][0]["User Id"];
                    ...
                    foreach ($newarr as $nts => $nstate) {
                        ...
                        if (!isset($reportdata[$id][$last]["End Time"])) {
                            ...
                            $reportdata[$id][$last]["End Time"]     = date("Y-m-d H:i:s", round($nts,2)/ 1000);
                            ...
                            if ($reportdata[$id][0]['lastts'] == $nts) {
                                ...
                                $reportdata[$id][$k]["End Time"] = date("Y-m-d H:i:s", round($reportdata[$id][0]['startts'],2)/ 1000);
                                ...
                                $reportdata[$id][$n]["Start Time"] = date("Y-m-d H:i:s", round($reportdata[$id][0]['startts'],2)/ 1000);
                        ...
                        $reportdata[$id][$k]["User Id"]   = $reportdata[$id][0]["User Id"];
                        // echo $nts."--2==<br>";
                        $reportdata[$id][$k]["Start Time"] = date("Y-m-d H:i:s", round($nts,2) / 1000);
                        $reportdata[$id][$k]['sip']        = $sip;
                        $reportdata[$id][$k]["Client Id"]  = "";
                        $reportdata[$id][$k]["Number"]     = "";
                    ...
                    $reportdata[$id][$k]["End Time"]     = date("Y-m-d H:i:s", round($pts,2)/ 1000);
                    $event_duration                         = date("H:i:s", (strtotime($reportdata[$id][$k]["End Time"]) - strtotime($reportdata[$id][$k]["Start Time"])));
                    $reportdata[$id][$k]["Event Length"] = $event_duration;
                    if (in_array($reportdata[$id][$last]["Mode"], $callMode)) {
                                $reportdata = calls($reportdata, $id, $last);
                    ...
                    $reportdata[$id][$k]["Start Time"]   = date("Y-m-d H:i:s", round($pts,2)/ 1000);
                    $reportdata[$id][$k]['sip']        = $sip;
                    $reportdata[$id][$k]["Client Id"]  = "";
                    $reportdata[$id][$k]["Number"]     = "";
                    $reportdata[$id][$k]["User Name"] = $reportdata[$id][0]["User Name"];
                ...
                if (empty($reportdata[$id][0]["Status"])) {
                    ...
                    $last = $k - 1;
                    if (!isset($reportdata[$id][$last]["End Time"])) {
                        $reportdata[$id][$last]["End Time"]     = date("Y-m-d H:i:s", round($pts,2)/ 1000);
                        $event_duration                         = date("H:i:s", (strtotime($reportdata[$id][$last]["End Time"]) - strtotime($reportdata[$id][$last]["Start Time"])));
                        $reportdata[$id][$last]["Event Length"] = $event_duration;
                        $reportdata                             = calls($reportdata, $id, $last);
