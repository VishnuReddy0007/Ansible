        ...
        foreach ($data as $sip => $sdata) {
            ...
            if (isset($sdata['states'])) {
                if(!empty($sdata['states'])){
                        ...
                        $covertstartdate=date("Y-m-d H:i:s",round($prets, 2) / 1000);
                        $covertenddate=date("Y-m-d H:i:s",round($ts, 2) / 1000);
                        $event_duration  = date("Y-m-d H:i:s",strtotime($covertenddate) - strtotime($covertstartdate));
                        $ulog[$previous] += strtotime($event_duration);
                        $previous   = ($states[0] == 'Paused') ? $states[1] : $states[0] . '-' . $states[1];
                        $prets      = $ts;
                    }
                }else{
                    $covertstartdate=date("Y-m-d H:i:s",round($prets, 2) / 1000);
                    $covertenddate=date("Y-m-d H:i:s",round($pts, 2) / 1000);
                    $event_duration  = date("Y-m-d H:i:s",strtotime($covertenddate) - strtotime($covertstartdate));
                    $ulog[$previous] += strtotime($event_duration);
